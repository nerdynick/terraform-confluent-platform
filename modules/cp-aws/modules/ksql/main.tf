...
# KSQL Resources
###########################
data "template_file" "ksql_name_prefix" {
...
resource "aws_instance" "ksql" {
    ...
    tags = merge(var.c3_tags, {"name"="${data.ksql_name_prefix.rendered}-${count.index+1}", "Name"="${data.ksql_name_prefix.rendered}-${count.index+1}"})
    volume_tags = merge(var.c3_tags, {"name"="${data.ksql_name_prefix.rendered}-${count.index+1}", "Name"="${data.ksql_name_prefix.rendered}-${count.index+1}"})
