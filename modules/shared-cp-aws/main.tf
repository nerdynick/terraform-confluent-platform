...
module "shared-cp-aws" {
    ...
    vpc_id = data.aws_vpc.shared_vpc.id
    subnet_ids = [data.aws_subnet.shared_subnet_a.id, data.aws_subnet.shared_subnet_b.id, data.aws_subnet.shared_subnet_c.id]
    multi_az = true
    image_id = var.image_id
    key_pair = var.key_pair
    enable_sg_creation = true
    dns_zone_id = data.aws_route53_zone.public.zone_id
    security_groups_ids = data.aws_security_groups.shared_sgs.ids
